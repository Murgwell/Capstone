...
public class Follower extends AbstractEnemy {
    ...
    public Follower(float x, float y, ScreenShake screenShake, PhysicsManager physics, NavMesh navMesh) {
        ...
        if (animDown == null) animDown = loadFolderAnim(
            "Textures/Enemies/World3/Follower/Run-Forward",
            "follower_walk-", 0, 5, 0.10f
        );

        animUp = loadAtlasAnim(
            "Textures/Enemies/World3/Follower/Run-Backward",
            "Follower_Run-Backward.atlas",
            "follower_walk-", 0.10f
        );
        ...
        animLeft = loadAtlasAnim(
            "Textures/Enemies/World3/Follower/Run-Left",
            "Follower_Run-Left.atlas",
            "follower_walk-", 0.10f
        );
        ...
        animRight = loadAtlasAnim(
            "Textures/Enemies/World3/Follower/Run-Right",
            "Follower_Run-Right.atlas",
            "follower_walk-", 0.10f
        );
        ...
        for (Texture tex : ownedTextures) {
            if (tex != null) {
                try {
                    ...
                    Gdx.app.error("Follower", "Failed to dispose texture: " + e.getMessage());
                }
            }
        }
        ownedTextures.clear();
        ...
        for (TextureAtlas atlas : ownedAtlases) {
            if (atlas != null) {
                try {
                    ...
                    Gdx.app.error("Follower", "Failed to dispose atlas: " + e.getMessage());
                }
            }
        }
        ownedAtlases.clear();
